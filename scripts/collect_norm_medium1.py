"""Collect norm for Medium demand - Part 1 (5 episodes)."""
...
def main():
    ...
    for i, route_file in enumerate(routes[:5]):
        ...
        # Handle reset result - may be dict of states
        episode_states = []
        ...
        env.close()
        print(f"[med1] [{i+1}/5] {Path(route_file).name} - {len(episode_states)} states")
    
    states_array = np.vstack(all_states)
    output = {"mean": np.mean(states_array, axis=0).tolist(), "std": np.std(states_array, axis=0).tolist(),
              "num_samples": len(states_array), "part": "medium1"}
    (project_root / "configs/norm_medium1.json").write_text(json.dumps(output, indent=2))
    print(f"Saved: configs/norm_medium1.json ({len(all_states)} samples)")

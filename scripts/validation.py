...
def validate_action_table(
    ...
    if state_dim in (10, 12, 14):  # Multi-agent mode (10D simplified, 12D local or 14D with global broadcast)
        # Allow flexible configs: 1×3=3 actions (final design), 1×5=5 actions, 1×9=9 actions, 3×5=15 actions
        valid_configs = [(1, 3), (1, 5), (1, 9), (3, 5), (3, 9)]
        if (expected_cycles, expected_splits) not in valid_configs:
            raise ValueError(
                f"Unsupported action config: {expected_cycles} cycles × {expected_splits} splits. "
                f"Valid configs are: {valid_configs}"
            )

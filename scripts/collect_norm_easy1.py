"""Collect norm for Easy demand - Part 1 (10 episodes)."""
...
def main():
    ...
    for i, route_file in enumerate(routes[:10]):
        ...
        env.close()
        print(f"[easy1] [{i+1}/10] {Path(route_file).name} - {len(episode_states)} states")
    
    states_array = np.vstack(all_states)
    output = {"mean": np.mean(states_array, axis=0).tolist(), "std": np.std(states_array, axis=0).tolist(),
              "num_samples": len(states_array), "part": "easy1"}
    (project_root / "configs/norm_easy1.json").write_text(json.dumps(output, indent=2))
    print(f"Saved: configs/norm_easy1.json ({len(all_states)} samples)")

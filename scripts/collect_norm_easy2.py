"""Collect norm for Easy demand - Part 2 (10 episodes)."""
...
def main():
    ...
    all_states = []
    for i, route_file in enumerate(routes[:10]):
        ...
        env.close()
        print(f"[easy2] [{i+1}/10] {Path(route_file).name} - {len(episode_states)} states")
    
    states_array = np.vstack(all_states)
    output = {"mean": np.mean(states_array, axis=0).tolist(), "std": np.std(states_array, axis=0).tolist(),
              "num_samples": len(states_array), "part": "easy2"}
    (project_root / "configs/norm_easy2.json").write_text(json.dumps(output, indent=2))
    print(f"Saved: configs/norm_easy2.json ({len(all_states)} samples)")

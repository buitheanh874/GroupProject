...
def test_templates_validate_action_table_and_scalars(template_path: Path):
    ...
    validate_scalar_params(
        ...
        rho_min=rho_min,
        g_min_sec=int(g_min_sec),
        lambda_fairness=sumo_cfg.get("lambda_fairness", 0.0),
        fairness_metric=sumo_cfg.get("fairness_metric", "max"),
        ...
        enable_anti_flicker=sumo_cfg.get("enable_anti_flicker", False),
        kappa=sumo_cfg.get("kappa", 0.0),
        enable_spillback_penalty=sumo_cfg.get("enable_spillback_penalty", False),
        beta=sumo_cfg.get("beta", 0.0),
        occ_threshold=sumo_cfg.get("occ_threshold", 0.0),
        allowed_cycles=allowed_cycles,
    )

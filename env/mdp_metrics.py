...
def compute_simple_wait_reward(
    wait_total: float,
    n_present: int,
) -> float:
    """
    Minimal reward: negative average waiting time per vehicle.

    Args:
        wait_total: Total waiting time (veh-sec) for the target scope (global or per TLS).
        n_present: Number of vehicles considered present in that scope (>=1).

    Returns:
        -wait_total / max(1, n_present)
    """
    n_veh = max(1, int(n_present))
    return -float(wait_total) / float(n_veh)


def compute_simple_clipped_reward(
    wait_total: float,
    n_present: int,
    t_ref: float = 60.0,
) -> float:
    """
    Simple normalized reward: -avg_wait / t_ref (NO CLIPPING).
    
    Removed clipping to preserve gradient signal for extreme cases.
    DQN with Huber loss is already robust to outliers.
    
    Args:
        wait_total: Total waiting time (veh-sec).
        n_present: Number of vehicles present (>=1).
        t_ref: Reference time for normalization (default 60s).
    
    Returns:
        -wait_total / (max(1, n_present) * t_ref)
    """
    n_veh = max(1, int(n_present))
    avg_wait = float(wait_total) / float(n_veh)
    reward = -avg_wait / float(t_ref)
    return reward

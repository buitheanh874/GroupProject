...
from env.mdp_metrics import (
    CycleMetricsAggregator,
    compute_normalized_reward,
    compute_normalized_reward_smdp,
    compute_simple_wait_reward,
    compute_simple_clipped_reward,
)
...
class SumoEnvConfig:
    ...
    reward_scope: str = "global"  # "global" (shared) or "local" (per-TLS)
    reward_mode: str = "smdp"     # "smdp" (default) or "simple" (avg wait only)
...
class SUMOEnv(BaseEnv):
    def __init__(
        ...
        if self._state_dim not in (4, 10, 12, 14):
            raise ValueError(f"state_dim must be 4, 10, 12, or 14, got {self._state_dim}")
        if len(self._tls_ids) > 1 and self._state_dim == 4:
            raise ValueError("state_dim must be 10, 12 or 14 when using multiple tls_ids")
        ...
        self._reward_scope = str(getattr(self._config, "reward_scope", "global")).strip().lower()
        if self._reward_scope not in {"global", "local"}:
            raise ValueError(f"reward_scope must be 'global' or 'local', got '{self._reward_scope}'")
        self._reward_mode = str(getattr(self._config, "reward_mode", "smdp")).strip().lower()
        if self._reward_mode not in {"smdp", "simple", "simple_clipped"}:
            raise ValueError(f"reward_mode must be 'smdp', 'simple', or 'simple_clipped', got '{self._reward_mode}'")
    ...
    def _step_multi(self, actions: Any) -> Tuple[Dict[str, np.ndarray], Dict[str, float], bool, Dict[str, Any]]:
        ...
        t_step_value = float(max_cycle_sec + 2 * int(self._config.yellow_sec) + 2 * int(self._config.all_red_sec))
        transition_total_sec = 2 * int(self._config.yellow_sec) + 2 * int(self._config.all_red_sec)
        spill_penalty = 0.0 if self._reward_mode == "simple" else self._compute_spillback_penalty()
        ...
        # SMDP REWARD v5 (FINAL) or SIMPLE: choose based on reward_mode
        # Objective: minimize avg waiting per vehicle
        ...
        # Compute spill scalar for broadcasting to state later (zero if simple)
        spill_scalar = float(spill_penalty)  # α * sum(occ^2)
        
        if self._reward_mode == "simple_clipped":
            # FINAL DESIGN: Simple reward r = -avg_wait/t_ref (no clipping)
            R_global = compute_simple_clipped_reward(
                wait_total=W_global,
                n_present=int(n_present),
                t_ref=T_REF,
            )
        elif self._reward_mode == "simple":
            R_global = compute_simple_wait_reward(wait_total=W_global, n_present=int(n_present))
        else:
        ...
        spill_scalar_norm = 0.0 if self._reward_mode == "simple" else (min(1.0, spill_scalar / (ALPHA * M)) if ALPHA * M > 0 else 0.0)

        for tls_id in self._tls_ids:
            if self._reward_scope == "local":
                n_local = max(1, int(np.sum(last_q_dir.get(tls_id, np.asarray([0.0], dtype=np.float32)))))
                if self._reward_mode == "simple":
                    rewards[tls_id] = float(
                        compute_simple_wait_reward(
                            wait_total=float(wait_totals.get(tls_id, 0.0)),
                            n_present=int(n_local),
                        )
                    )
                else:
                    rewards[tls_id] = float(
    ...
    def _resolve_action_splits(self) -> List[Tuple[float, float]]:
        ...
        # Allow flexible split configurations
        if len(splits) == 0:
    ...
    def _resolve_cycle_options(self) -> List[int]:
        ...
        # Allow flexible cycle configurations
        if len(cycles) == 0:
            raise ValueError("cycle_options_sec must not be empty")
    ...
    def _build_action_definitions(self) -> List[SumoActionDefinition]:
        ...
        # Support multiple action space configurations
        valid_configs = [
            (3, 5, 15),  # Standard: 3 cycles × 5 splits = 15 actions
            (1, 9, 9),   # 9-button baseline: 1 cycle × 9 splits = 9 actions
            (1, 5, 5),   # Simple: 1 cycle × 5 splits = 5 actions (for easier learning)
            (1, 3, 3),   # Final design: 1 cycle × 3 splits = 3 actions
        ]
